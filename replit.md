...
- **Interview Scheduling**: `interview_slots` table with interviewers (JSON), scheduledAt, durationMinutes, type, status (Scheduled/Completed/Cancelled/No-Show), feedback, rating; full REST API (CRUD, /complete, /cancel, /noshow, /upcoming); `/interviews` page with day-grouped timeline, Scheduled/Completed/Cancelled/All/Upcoming filter pills, CompleteDialog with 5-star rating + feedback; scheduling panel in ticket detail showing per-ticket interviews + "Schedule" button; fires `interview.scheduled` webhook event
